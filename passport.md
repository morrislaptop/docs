...
When using this method of authentication, Axios will automatically send the `laravel_token` cookie. In addition, the default Laravel JavaScript scaffolding instructs Axios to send the `X-Requested-With` and `X-CSRF-TOKEN` headers:
...
> {note} If you are using a different JavaScript framework, you should make sure it is configured to send the `laravel_token` cookie, the `X-CSRF-TOKEN` and `X-Requested-With` headers with every outgoing request.
